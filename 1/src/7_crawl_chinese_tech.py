...
ZH_TECH_FILENAME = os.path.join(OUTPUT_DIR, "chinese_data_tech.txt")
#  增加多个起始URL，为爬虫提供更广阔的探索空间
#   这边不知道为什么一直下载不成功， 所以添加了一堆入口突然好了
START_URLS = [
    "https://tech.sina.com.cn/",
    "https://tech.sina.com.cn/internet/",
    "https://tech.sina.com.cn/mobile/",
    "https://tech.sina.com.cn/digi/",
    "https://tech.sina.com.cn/notebook/",
    "https://tech.sina.com.cn/apple/"
]
TARGET_SIZE_MB = 2.0
MAX_WORKERS = 20
...
# --- 爬虫逻辑 ---
# 只匹配科技新闻链接
LINK_PATTERN = r'https?://tech\.sina\.com\.cn/.*?/\d{4}-\d{2}-\d{2}/doc-.*\.shtml'
ARTICLE_BODY_SELECTORS = ['.article-content-left', '#article', '.main-content', '.article-body', '.art_content']
...
def worker():
    while not stop_event.is_set():
        ...
        if article_text:
            with file_lock:
                with open(ZH_TECH_FILENAME, 'a', encoding='utf-8') as f: f.write(article_text + '\n\n')
                current_size_mb = os.path.getsize(ZH_TECH_FILENAME) / (1024 * 1024)
...
def crawl_tech_data():
    ...
    if os.path.exists(ZH_TECH_FILENAME): os.remove(ZH_TECH_FILENAME)
    ...
    print(f"科技新闻数据爬取完成。最终文件大小: {os.path.getsize(ZH_TECH_FILENAME) / (1024*1024):.2f} MB")
